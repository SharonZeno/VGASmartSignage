...
//VGA Smart Signage did this!
void _waitForXMinute(double x) {
    time_t start = time(NULL);  // Get the current time

    while (true) {
        time_t current = time(NULL);  // Get the current time again
        double elapsed = difftime(current, start);

        if (elapsed >= x)  // Check if 1 minute has elapsed
            break;
   }
}

//VGA Smart Signage did this!
int uiApp::runForOneMinute(BitmappedDisplayController * displayController)
{
   m_displayController = displayController;
  m_displayColors     = displayController->colorsCount();
  ...
  while (true) {
    ...
    if(uxQueueMessagesWaiting(m_eventsQueue) == 0)
    {
      _waitForXMinute(60);
      break;
    }
    ...
    if (getEvent(&event, -1)) {
      ...
      // if (elapsed >= 20.0)  // Check if 1 minute has elapsed
      //     // minuteNotDone = false;
      //     quit(1);
    }
  }
